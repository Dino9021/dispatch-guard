...
| the `Stop` hook does not fire on a user interrupt; a Bash write whose HANDOFF.md path is hidden in a shell variable is not recorded; the record holds a folder NAME, so a same-named folder in another tree could satisfy it | the next PACE/STOP prompt arms; the file tool is the documented route (unseen = nothing armed, logged); two arms for one folder de-duplicate through `resume.json` |
