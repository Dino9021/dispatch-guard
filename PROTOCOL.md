...
| the wind-down does not fire for a FAILED tool call | `PostToolUse` does not run for one and the gate does not register `PostToolUseFailure`. ⚠ The note is on `PreToolUse` precisely so a failing loop still hears it, but a tool that fails after its `PreToolUse` produces no second chance |
