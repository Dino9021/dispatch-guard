...
| a **rewrite of a file that declares itself append-only** is refused — `Write`, `Edit`, `MultiEdit` and the shell alike (0.63.0, the `cowork` skill's rule 3) | `guard_append_only` | the file's first heading contains `append-only` / `append only`, or it carries `<!-- append-only -->`, within its first 2 048 bytes — body text does not count. A `Write` must start with the file's current text; an `Edit`'s `old_string` must be the end of the file, its `new_string` must start with it, and an `old_string` that occurs more than once is refused with or without `replace_all` (the tool edits one occurrence; the guard cannot tell which); in the shell, `> path` (quoted paths included), `sed -i`, `tee` without `-a`, `rm`, `truncate`, `cp` **onto** it, `mv` **from or onto** it, `Set-Content`, `Out-File` without `-Append`, `Clear-Content`, `Remove-Item`, `Move-Item`, `Rename-Item`, `Copy-Item` **onto** it, with a leading `cd x &&` REPLACING the cwd when the path is resolved. `>>`, `tee -a`, `Add-Content`, reads and copies **out** pass; an unreadable file is logged `CMD-ALLOW(guard_append_only unreadable)` and allowed. A token the gate cannot expand (`$VAR`, `~`, a glob) is logged `CMD-ALLOW(guard_append_only unresolved)` and allowed |
| the **first write or commit** of a session that shares this repository with a **live peer** is refused **once**, until `cowork` is invoked (0.63.0) | `guard_cowork_first`, `peer_alive_min` (15) | another session's `state/<sid>.alive` younger than `peer_alive_min` minutes, whose recorded start `cwd` resolves (`normcase`d) to this call's repository root. Refused once per session (`state/<sid>.cowork-nagged`), then allowed; silent once the `Skill` call was recorded; the off switch logs `CMD-DISABLED` and does not spend the mark. Sub-agents share the parent's id and are never peers |
...
| only the FIRST segment's `cd` is honoured, and only the `cd` spelling: `cd a && cd .. && …`, `pushd x && …`, `cd -- x && …` and `(cd x && …)` resolve against the payload cwd | the second `cd` and the other spellings are unseen — a miss, never a false refusal (review C, b3). A quoted redirect path containing `;` or `\|` is split by the operator splitter before the redirect regex sees it (review C, a1) |
| a file whose first line is `---` as a Markdown horizontal rule IS marked when its H1 follows (the frontmatter skip requires a YAML key on line 2) — but a file whose real YAML frontmatter has no closing `---` is read from its top | the `<!-- append-only -->` comment is the escape hatch that ignores both (review C, e1) |
