...
| `HANDOFF.md` | written on STOP; the resume's only input. ⛔ **It must say WHO the session is** — a role or call sign AND the session id. A resume wakes a FRESH `claude -p` with a NEW id (never `--resume`: ~95k tokens/MB at zero cache read), so the successor can only keep a name this file gives it. `handoff_warnings()` says so when it is missing |
...
| a **rewrite of a file that declares itself append-only** is refused — `Write`, `Edit`, `MultiEdit` and the shell alike (0.63.0, the `cowork` skill's rule 3) | `guard_append_only` | the file's first heading contains `append-only` / `append only`, or an `<!-- append-only -->` comment stands on a line of its own, within its first 2 048 bytes — body text does not count, and neither does the comment mentioned inline (0.63.1: the 0.63.0 rule marked the cowork skill's own reading copy, whose rule 3 quotes the comment in backticks). A `Write` must start with the file's current text; an `Edit`'s `old_string` must be the end of the file, its `new_string` must start with it, and an `old_string` that occurs more than once is refused with or without `replace_all` (the tool edits one occurrence; the guard cannot tell which); in the shell, `> path` (quoted paths included), `sed -i`, `tee` without `-a`, `rm`, `truncate`, `cp` **onto** it, `mv` **from or onto** it, `Set-Content`, `Out-File` without `-Append`, `Clear-Content`, `Remove-Item`, `Move-Item` / `Rename-Item` (every operand, positional or named), `Copy-Item` **onto** it, plus `del`, `erase`, `move`, `copy` and the `2>` / `&>` redirects, with every leading `cd` / `Set-Location` / `pushd` REPLACING the cwd in turn when the path is resolved. An `Edit`'s single anchor occurrence must also be at the END of the file (a raw-unique anchor that is an earlier line would insert mid-file). `>>`, `tee -a`, `Add-Content`, reads and copies **out** pass; an unreadable file is logged `CMD-ALLOW(guard_append_only unreadable)` and allowed. A token the gate cannot expand (`$VAR`, `~`, a glob) is logged `CMD-ALLOW(guard_append_only unresolved)` and allowed |
| the **first write (any file tool) or commit** of a session that shares this repository with a **live peer** is refused **once**, until `cowork` is invoked (0.63.0) | `guard_cowork_first`, `peer_alive_min` (15) | another session's `state/<sid>.alive` younger than `peer_alive_min` minutes, whose recorded start `cwd` resolves (`normcase`d) to this call's repository root. Refused once per session (`state/<sid>.cowork-nagged`), then allowed; silent once the `Skill` call was recorded; the off switch logs `CMD-DISABLED` and does not spend the mark. Sub-agents share the parent's id and are never peers |
...
| the gate is per session | deliberate. Two sessions sharing one working tree is the `cowork` skill's subject — **one tree, one owner of the irreversible operations, a throwaway clone for a commit that cannot land here**; never a worktree per session (the owner's rule, and until 0.62.0 the branch guard's message said the opposite). The two cowork guards above cover the two rules a hook can see. ⛔ Across MACHINES they cover nothing: the heartbeat is a file in THIS machine's state directory, so a session on another machine is never a peer — the cross-machine rules (`cowork` rule 13, `reference/cross-machine.md`, 0.64.0) are advice with no gate behind them |
| a marked append-only file cannot be truncated, rewritten in place, moved or removed **by name** through the tools; a **copy inherits the marker**, so a second snapshot to the same name is refused | that is the rule — snapshot to a NEW name; the owner's own editor is unaffected, this is a hook on Claude Code's tool calls only |
| a program that rewrites an append-only file in place (`python fix.py board.md`, `dd of=`), removing the DIRECTORY that holds it (`rm -rf board`), or `cp X board/` / `mv X board/` with a directory destination, all pass unseen | the guard reads shell operators and file-tool inputs, never what a program does or what a directory contains — measured by code reviews A and C, 2026-09-21 |
| every LEADING `cd` / `chdir` / `sl` / `Set-Location` / `pushd` segment is applied in turn (case-insensitive) and REPLACES the directory the path resolves against; `cd -- x`, a subshell `(cd x && …)`, and a `cd` that FAILS followed by `;` or `\|\|` (the rest then runs in the OLD cwd) are unseen | a miss in the old cwd, never a false refusal (reviews C b3, D D3/D4/D5). A quoted redirect path containing `;` or `\|` is split by the operator splitter before the redirect regex sees it (review C, a1) |
| the frontmatter skip fires only when the block looks like YAML — after the opening `---`, blank and `#` comment lines, then a `key:` line; a Markdown `---` rule on line 1 followed by the marked H1 is therefore NOT skipped and the file IS marked. ⚠ An UNCLOSED frontmatter runs to the first later `---` line inside the 2 048-byte head and swallows what is between — on a `---`-separated board that can be the H1 | the `<!-- append-only -->` comment is the escape hatch that ignores all of it (reviews C e1, D D7/D8) |
| PowerShell's short aliases `ri`, `mi`, `rni`, `cpi`, `sc`, `clc` are unseen; `del`, `erase`, `move`, `copy` and the `2>` / `&>` redirects ARE seen but the user documents (SKILL.md) name only the shapes a reader would look for | the shell-string guard covers spelled-out cmdlets and POSIX names; the log names every allow (review D, D11) |
| the peer test sees only SAME-MACHINE peers — `.alive` lives in this machine's state directory, so a session on another machine never nags (0.64.0); within one machine it sees this session's root from the payload `cwd` (which follows `cd`) and a peer's from its START cwd; `.alive` files are pruned by count (20) at session start; an exited peer stays "live" for up to `peer_alive_min` | false negatives in a marker-less tree and beyond 20 sessions; one nag against a peer that just left — once, and R2/R5 in the ADR say what would change it |
...
| the `Stop` hook does not fire on a user interrupt; a Bash write whose HANDOFF.md path is hidden in a shell variable is not recorded; the record holds a folder NAME, so a same-named folder in another tree could satisfy it | the next PACE/STOP prompt arms; the file tool is the documented route (unseen = nothing armed, logged); two arms for one folder de-duplicate through this session's `resume/<session>.json` |
